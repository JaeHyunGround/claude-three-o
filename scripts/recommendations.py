...
RECOMMENDATION_CATALOG = {
    ...
    "geo": {
        ...
        "add_llms_txt": {
            "condition": lambda d: d.get("platforms", {}).get("chatgpt", {}).get("score", 100) < 30 or d.get("platforms", {}).get("perplexity", {}).get("score", 100) < 30,
            "title": "Consider llms.txt for non-Google AI platforms",
            "detail": "llms.txt may help ChatGPT/Perplexity/Claude discover key pages. Note: Google officially states llms.txt is NOT required for Google AI search features (AI Overviews, AI Mode).",
            "effort": "low",
            "impact": "low",
        },
        "create_non_commodity_content": {
            "condition": lambda d: d.get("score", 100) < 55,
            "title": "Create non-commodity content with unique perspective",
            "detail": "Replace generic 'tips' content with first-hand experience, original data, or expert analysis. Google AI systems prioritize unique viewpoints over recycled common knowledge. Focus on what only you can say based on direct experience.",
            "effort": "high",
            "impact": "high",
        },
    },
    "aao": {
        "add_json_ld": {
            ...
            "detail": "Add Schema.org markup matching your business type (LocalBusiness, Product, Service). Include name, address, hours, pricing. Note: structured data is not required for Google AI search specifically, but enables rich results and helps AI agents discover actionable data.",
            "effort": "medium",
            "impact": "critical",
        },
        "add_schema_actions": {
